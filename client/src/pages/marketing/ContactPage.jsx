...
export function ContactPage() {
  ...
  const inputStyle = {
    ...
    color: "var(--color-text)", fontSize: 14, fontFamily: "'DM Sans', system-ui",
    ...
    <div className="scroll-dock-page" style={{ minHeight: "100vh", display: "flex", flexDirection: "column",
      ...
      <main style={{ flex: 1, maxWidth: 560, margin: "0 auto", padding: "64px 24px" }}>
        <h1 style={{ fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 900, letterSpacing: "-1.5px",
                      color: "var(--color-text)", marginBottom: 12, lineHeight: 1.1,
                      fontFamily: "'Barlow Condensed', 'DM Sans', system-ui" }}>
          Get in touch
        </h1>
        <p style={{ fontSize: 15, color: "var(--color-text-muted)", lineHeight: 1.6, marginBottom: 40 }}>
          We typically respond within 24 hours.
        </p>

        {status === "ok" ? (
          <div style={{ padding: "24px 28px", background: "rgba(34, 197, 94, 0.12)",
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {status === "error" && (
              <div style={{ color: "var(--color-destructive)", fontSize: 13 }}>
