/**
 * Company -> logo URL, with no network and no server dependency (TASK AE5).
 *
 * MOVED HERE, NOT COPIED. This table and the two pure functions over it were the top half of
 * services/jobs/enrichLogos.js, which the board could not import: that module pulls in axios for its
 * HEAD-verified `fetchLogoUrl`, and a second copy of a 60-entry domain table is precisely the
 * failure mode this codebase keeps naming - one half of the app resolving a company differently from
 * the other. enrichLogos.js now imports from here, so there is still exactly one table, and
 * `fetchLogoUrl` stays where the network lives.
 *
 * WHY THE BOARD NEEDS IT. JobCard has always rendered CompanyIcon, and CompanyIcon has always had a
 * lettered-tile fallback - the missing piece was a URL. `scraped_jobs.company_icon_url` is only
 * populated by the writers whose feed happens to carry a logo (LinkedIn's, serpapi's thumbnail), so
 * every jobo/greenhouse/ashby row arrived with null and every one of those cards showed a letter.
 * Resolving the known domain client-side fills that in without a request per card.
 *
 * A GUESS IS NOT A LOGO. getKnownLogoUrl returns null unless the company matches a domain we
 * actually know. companyToDomain's slug fallback exists for the enrichment path, which verifies with
 * a HEAD request; handing the same guess to an <img> would render a broken image for every company
 * not in the table.
 */
