...
// all of them. JobsPanel stays in the list because the board still owns the queue button that feeds
// the pipeline — and because an assertion that silently stopped covering anything would be worse
// than one that fails.
//
// AC2 added the fourth: the modal's per-attempt row moved into AutoApplyPanelSections.jsx as
// AttemptRow when the modal was restructured into company -> application -> problems. Its controls
// are unchanged, so the assertions are unchanged; only the file holding them moved, which is
// exactly the case this list exists to absorb.
const jobsPanel = [
