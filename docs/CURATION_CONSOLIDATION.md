...
| ~~**`ats_term_weights` go stale 2026-10-13**~~ **DONE 2026-09-18** | `docs/ATS_TERM_WEIGHTS_SCHEDULE.md`. And the premise was local: **production's table was EMPTY and always had been**, so there was no deadline — every score it ever served was unweighted. The `console.warn` was gated on `weights.size`, so an empty table was silent *by construction*. Now: `REFRESH_WEIGHT_AGE_DAYS = 14` inside the 45-day refusal, rebuilt at boot and after the nightly crawl, and all three states (fresh/stale/**absent**) reported in the boot log and on `/api/version`. Measured cost of the unweighted mode it was running in: 20.4% of board scores differ by ≤5 points, 2.8% change band, ρ 0.476 → 0.415. |
